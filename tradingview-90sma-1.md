...
basePosition = input(defval = 8, title = "From Month", minval = 1, maxval = 10000) // 首次建仓开单数量
...
var fakeCrossNumMax = input(defval = 1, title = "fake cross num target", minval = 1, maxval = 10) // 表示假突破目标值
var fakeCrossNum = 0.0 // 表示假突破次数
var fakeCrossNumTarget = input(defval = 1, title = "fake cross num target", minval = 1, maxval = 10) // 表示假突破目
...
if window()
    // K线再次破线，重置
    if crossOverCondition and crossUnderCondition and formerCross > 0.0
        //重置第一条件，回到默认值，
        formerCross := 0.0
        fakeCrossNum = 0
    ...
    //记录加突破次数
    //当下穿, 高点大于90线，收盘小于90线 
    if formerCross == 1.0 and curHighPrice > curCloseAnchorPrice and curClosePrice < curCloseAnchorPrice
        fakeCrossNum += 1.0

    //记录加突破次数
    //当上穿, 低点小于90线，收盘大于90线 
    if formerCross == 1.0 and curLowPrice < curCloseAnchorPrice and curClosePrice > curCloseAnchorPrice
        fakeCrossNum += 1.0    
